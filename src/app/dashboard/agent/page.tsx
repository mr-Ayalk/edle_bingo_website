...
export default function AgentDashboardPage() {
  ...
  const generate = async (e: React.FormEvent) => {
    ...
    setUser(data.user);
    signalAlert('success', 'Voucher generated.');
    setAmount('');
    setCode('');
          ...
          <div className="dashboard-metrics">
            <div className="metric-card bg-gradient-info">
              <div className="metric-card-body">
                ...
                <h2 className="metric-value">${user.balance.toLocaleString()}</h2>
              </div>
        ...
        <div className="card narrow-card">
          <div className="card-body">
            <form onSubmit={generate} className="form-layout">
              <label>{tr('yourCredit')}</label>
              <input className="form-control" readOnly value={`$${user.balance.toLocaleString()}`} />
              <label>{tr('voucherAmount')}</label>
              <input className="form-control" type="number" step="0.01" min="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} required />
              <label>{tr('customCode')}</label>
