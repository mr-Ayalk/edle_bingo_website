...
import { downloadStorageDir, storageFilePath } from '@/lib/download-storage';
...
export async function POST(request: Request) {
  ...
  if (process.env.VERCEL === '1') {
    return NextResponse.json(
      {
        message:
          'File uploads are not supported on Vercel serverless. Use blob storage (S3/R2/Vercel Blob) or deploy on a VPS.',
      },
      { status: 503 },
    );
  }
  ...
  try {
    ...
    const existingCount = await prisma.$transaction(async (tx) => {
      const count = await tx.downloadAsset.count({ where: { category } });
      if (meta.maxFiles !== null && count >= meta.maxFiles) {
        throw new Error('MAX_FILES');
      }
      return count;
    }).catch((err) => {
      if (err instanceof Error && err.message === 'MAX_FILES') return -1;
      throw err;
    });
    ...
    const absDir = downloadStorageDir(category);
    await mkdir(absDir, { recursive: true });

    const buffer = Buffer.from(await file.arrayBuffer());
    absPath = path.join(absDir, storedName);
    await writeFile(absPath, buffer);

    const filePath = storageFilePath(category, storedName);
